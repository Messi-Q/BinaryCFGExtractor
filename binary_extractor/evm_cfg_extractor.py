...
def main() -> None:
    ...
    if args.explore or args.address:
        ...
        if args.rpc:
            from binary_extractor.platforms.ETH.explorer import EthereumExplorerRPC
            # parsing RPC HOST & PORT
            host, port = args.rpc.split(':')
            octo_explorer = EthereumExplorerRPC(host=host, port=port, tls=args.rpctls)
        else:
            from binary_extractor.platforms.ETH.explorer import EthereumInfuraExplorer
            from binary_extractor.platforms.ETH.explorer import (INFURA_MAINNET,
                                                                 INFURA_ROPSTEN,
                                                                 INFURA_INFURANET,
                                                                 INFURA_KOVAN,
                                                                 INFURA_RINKEBY)
